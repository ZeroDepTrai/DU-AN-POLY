            ...
            {lowStock.map((p) => (
              <div key={p.id} className="flex items-center gap-3 rounded-xl border border-gunmetal/30 bg-cardoverlay p-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-crimson">Ch? còn {p.stock} s?n ph?m</p>
                </div>
              </div>
            ))}
          </div>
...
function ProductsTab({ products }: { products: Product[] }) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data?.soft_deleted) {
        setInfo(
          data.message ||
            `S?n ph?m ?ã ???c ?n kh?i c?a hàng (còn ${data.order_items ?? 0} ??n hàng tham chi?u nên không th? xóa hoàn toàn ?? gi? l?ch s?).`,
        );
      } else {
  ...
  const handleDelete = async (p: Product) => {
    const ok = window.confirm(
      `Xóa "${p.name}"?\n\n` +
        `• N?u s?n ph?m ch?a t?ng ???c ??t hàng, s? b? xóa hoàn toàn kh?i database.\n` +
        `• N?u ?ã có ??n hàng tham chi?u, s?n ph?m s? ???c ?N (gi? l?ch s? ??n).\n\n` +
        `Hành ??ng này KHÔNG TH? hoàn tác ??i v?i xóa hoàn toàn.`,
    );
    if (!ok) return;
    ...
    <div>
      ...
      <div className="mb-4 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          />
          <span className="text-[11px] text-softgray">(t? ??ng b?t khi ch?n danh m?c / t??ng thích bên d??i)</span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="self-center text-xs font-semibold uppercase tracking-wider text-softgray">• T??ng thích</span>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 min-h-[32px]">
              ...
              {quickTagChips.length === 0 && (
                <span className="text-xs text-softgray italic">Nh?n nhãn bên trên ?? ch?n...</span>
              )}
            </div>
          </div>
      ...
      {formTab === "full" && (
        ...
        >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 min-h-[36px]">
              ...
              {fullTagChips.length === 0 && (
                <span className="text-xs text-softgray italic self-center">Nh?n nhãn bên trên ?? ch?n...</span>
              )}
            </div>
          </div>
          ...
          <div>
            <label className="mb-1.5 block text-sm text-softgray">Hình ?nh {editing ? "(b? tr?ng ?? gi? hình c?)" : ""}</label>
            <input
              type="file"
              accept="image/*"
        ...
        <div className="lg:col-span-2 admin-form-card space-y-4">
          ...
          {selectedOrder ? (
            ...
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <p className="text-sm text-softgray">Ch?n m?t ??n hàng ?? c?p nh?t v? trí giao hàng.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="admin-form-card mb-6 space-y-4">
        ...
        <div className="flex gap-3">
          <button onClick={handleSubmit} disabled={saveMutation.isPending} className="btn-primary">
            {saveMutation.isPending ? "?ang l?u..." : editing ? "C?p nh?t bài vi?t" : "??ng bài vi?t"}
          </button>
          {editing && (
...
function SettingsTab() {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setNewEmail(""); setAddSuccess("?ã thêm email thành công."); setAddError("");
      setTimeout(() => setAddSuccess(""), 3000);
    },
    onError: (err: unknown) => {
      const msg = (err as { response?: { data?: { detail?: string } } }).response?.data?.detail ?? "Thêm th?t b?i.";
      setAddError(msg);
      setAddSuccess("");
    },
  });
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="admin-form-card p-6">
        ...
        {emails.length === 0 ? (
          <p className="py-6 text-center text-sm text-softgray">Ch?a có email nào ???c thêm.</p>
        ) : (
            ...
            {emails.map((e) => (
              <li key={e.id} className="flex items-center justify-between py-3">
                ...
                <button onClick={() => deleteMutation.mutate(e.id)}
                  className="ml-4 flex-shrink-0 rounded-lg px-3 py-1.5 text-xs text-blushlight hover:bg-crimson/10 transition-colors">Xóa</button>
              </li>
            ))}
          </ul>
        )}
        ...
        <form onSubmit={(e) => { e.preventDefault(); if (newEmail.trim()) addMutation.mutate(newEmail.trim()); }}
          ...
          <button type="submit" disabled={addMutation.isPending || !newEmail.trim()}
            ...
            {addMutation.isPending ? "?ang thêm..." : "Thêm email"}
          </button>
        </form>
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
        <div className="admin-form-card p-4">
          ...
          <div className="max-h-[64vh] overflow-y-auto space-y-1.5">
            {filtered.length === 0 ? (
              <p className="py-6 text-center text-xs text-softgray">Không tìm th?y.</p>
            ) : (
              filtered.map((p) => (
                <button
                  key={p.id}
...
function ProductGalleryEditor({ product }: { product: Product }) {
  ...
  const deleteMedia = (m: ProductMediaItem) => {
    if (!confirm("Xóa hình ?nh/video này?")) return;
    ...
    <div className="admin-form-card p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-softgray">Mã s?n ph?m: #{product.id}</p>
        </div>
      ...
      >
        ...
        <span className="text-sm font-medium text-warmwhite">
          {uploading ? "?ang t?i lên..." : "Kéo th? ho?c nh?n ?? ch?n ?nh/video"}
        </span>
        ...
        <span className="mt-2 inline-flex items-center gap-2 text-xs text-softgray" onClick={(e) => e.stopPropagation()}>
          ...
          ??t file t?i lên ??u tiên làm ?nh cover
        </span>
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-gunmetal/30 bg-gunmetal/20 p-8 text-center text-sm text-softgray">
          S?n ph?m này ch?a có ?nh/video trong gallery. Hi?n ?ang dùng ?nh thumbnail m?c ??nh.
        </div>
      ) : (
  ...
  return (
    <div>
      ...
      <div className="admin-form-card mb-6 space-y-3">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div>
            <label className="mb-1 block text-xs text-softgray">Mã (CODE)</label>
            <input
              disabled={!!editing}
              value={form.code}
          ...
          <div>
            <label className="mb-1 block text-xs text-softgray">Giá tr? {form.discount_type === "percent" ? "(1-100)" : "(VND)"}</label>
          ...
          <div>
            <label className="mb-1 block text-xs text-softgray">??n hàng t?i thi?u (VND)</label>
          ...
          <div>
            <label className="mb-1 block text-xs text-softgray">Gi?m t?i ?a (VND, không b?t bu?c)</label>
          ...
          <div>
            <label className="mb-1 block text-xs text-softgray">S? l??t t?i ?a (0 = không gi?i h?n)</label>
          ...
          <div>
            <label className="mb-1 block text-xs text-softgray">B?t ??u (tùy ch?n)</label>
          ...
          <div>
            <label className="mb-1 block text-xs text-softgray">H?t h?n (tùy ch?n)</label>
        ...
        <div className="admin-form-card overflow-hidden p-0">
          <table className="admin-table">
            <thead className="admin-table-header">
              <tr>
                <th className="px-4 py-3">Mã</th>
                <th className="px-4 py-3">Mô t?</th>
                <th className="px-4 py-3">Gi?m</th>
                <th className="px-4 py-3">T?i thi?u</th>
                <th className="px-4 py-3">?ã dùng / Gi?i h?n</th>
                <th className="px-4 py-3">Tr?ng thái</th>
                <th className="px-4 py-3">Thao tác</th>
              </tr>
            </thead>
