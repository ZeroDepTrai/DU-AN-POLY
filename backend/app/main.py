...
async def lifespan(app: FastAPI):
    ...
    with engine.connect() as conn:
        try:
            ...
            _force_add(conn, "orders", "created_at",
    ...
    # Background image maintenance runs sequentially on one daemon thread:
    # first migrate legacy JPEG/PNG URLs to WebP, then clear truly broken URLs.
    # A slow persistent volume therefore cannot delay the API binding its port.
    try:
        from app.services.legacy_image_migration import maintain_image_urls_async
        maintain_image_urls_async()
    except Exception as e:
        logger.warning(f"[IMAGE MAINTENANCE] could not schedule: {e}")
